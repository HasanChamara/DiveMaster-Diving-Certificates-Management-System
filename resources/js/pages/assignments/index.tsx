...
export default function Assignments() {
    ...
    const destroyAssignment: FormEventHandler = (e, id) =>{
        e.preventDefault();
        if (confirm("Are you sure you want to delete this Assignment ?")){
            destroy(route('assignments.destroy', id));
        }
    }
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div>
                ...
                <div className="overflow-x-auto">
                    ...
                    {assignments.map(({id, title, mark, level, question}) =>(
                        <tbody key={id}>
                        ...
                        <td className="px-6 py-2 text-gray-600 dark:text-gray-300">{mark}</td>
                        <td className="px-6 py-2 text-gray-600 dark:text-gray-300">{level}</td>
                        <td className="px-6 py-2 text-gray-600 dark:text-gray-300">{question}</td>
                        <td className="px-6 py-2 flex gap-2">
                            <form onSubmit={(e => destroyAssignment(e, id))}>
                        <br></br>

                        <Link  href={route('assignments.edit', id)}
                            className="px-3 py-2 text-xs font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800"
                        >
                            Edit Assignment
                        </Link>
